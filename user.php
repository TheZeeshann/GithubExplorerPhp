<?php 
  ...
  $api = new Api;
  $rmtAdrs = $_SERVER['REMOTE_ADDR'];
  $apiCallLimit = <<<HERE
      <div class="center">
        <br><br><br><br>
        <i class="material-icons large socialcodia">tag_faces</i>
        <h1><b>Api Call Limit Exceeded </b></h1>
        <h3><b>For Your Ip $rmtAdrs</b></h3>
    </div>
  HERE;;
?>
...
<?php
    ...
    if($_GET['username'] || strlen($_GET['username'])>1)
    {
      ...
      $response = $api->getUser($username);
      $followersResponse = $api->getFollowers($username);
      $followingResponse = $api->getFollowings($username);
      $repositoryResponse = $api->getRepository($username);
